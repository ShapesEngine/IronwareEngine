...
// Data Type helpers
static __forceinline int       DataTypeFormatString(char* buf, int buf_size, ImGuiDataType data_type, const void* data_ptr, const char* format);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
    ...
    if (hovered)
        window->DrawList->AddCircleFilled(center, ImMax(2.f, radius), GetColorU32(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered), 9);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    ImRect bb(pos, pos + ImVec2(g.FontSize, g.FontSize) + g.Style.FramePadding * 2.f);
    ItemAdd(bb, id);
    bool hovered, held;
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    float alpha = 1.f;
    if ((axis == ImGuiAxis_Y) && bb_height < g.FontSize + g.Style.FramePadding.y * 2.f)
    {
        alpha = ImSaturate((bb_height - g.FontSize) / (g.Style.FramePadding.y * 2.f));
        if (alpha <= 0.f)
            return;
    }
    ...
    bb.Expand(ImVec2(-ImClamp((float)(int)((bb.Max.x - bb.Min.x - 2.f) * 0.5f), 0.f, 3.f), -ImClamp((float)(int)((bb.Max.y - bb.Min.y - 2.f) * 0.5f), 0.f, 3.f)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.f ? style.ItemInnerSpacing.x + label_size.x : 0.f), label_size.y + style.FramePadding.y * 2.f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (*v)
    {
        const float pad = ImMax(1.f, (float)(int)(square_sz / 6.f));
        RenderCheckMark(check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark), square_sz - pad*2.f);
    }
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImRect bb(pos, pos + CalcItemSize(size_arg, CalcItemWidth(), g.FontSize + style.FramePadding.y*2.f));
    ...
    if (!overlay)
    {
        ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), "%.f%%", fraction*100+0.01f);
        overlay = overlay_buf;
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.f));
...
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f')
    {
        ...
        return g.TempBuffer;
#else
        IM_ASSERT(0 && "DragInt(): Invalid format string!"); // Old versions used a default parameter of "%.f", please replace with e.g. "%d"
#endif
    }
    return fmt;
}
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    // Patch old "%.f" format string to use "%d", read function comments for more details.
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float slider_sz = (bb.Max[axis] - bb.Min[axis]) - grab_padding * 2.f;
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    case ImGuiDataType_Float:
        IM_ASSERT(*(const float*)v_min >= -FLT_MAX/2.f && *(const float*)v_max <= FLT_MAX/2.f);
    ...
    case ImGuiDataType_Double:
        IM_ASSERT(*(const double*)v_min >= -DBL_MAX/2.f && *(const double*)v_max <= DBL_MAX/2.f);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? GetTextLineHeight() * 8.f : label_size.y) + style.FramePadding.y*2.f); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.f ? (style.ItemInnerSpacing.x + label_size.x) : 0.f, 0.f));
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, GetColorU32(ImGuiCol_Text), buf_display, buf_display_end, 0.f, is_multiline ? NULL : &clip_rect);
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, GetColorU32(ImGuiCol_Text), buf_display, buf_display_end, 0.f, is_multiline ? NULL : &clip_rect);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            {
                value_changed |= DragFloat(ids[n], &f[n], 1.f/255.f, 0.f, hdr ? 0.f : 1.f, fmt_table_float[fmt_idx][n]);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            PushItemWidth(square_sz * 12.f); // Use 256 + bar sizes?
...
static __forceinline ImU32 ImAlphaBlendColor(ImU32 col_a, ImU32 col_b)
{
...
void ImGui::RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max, ImU32 col, float grid_step, ImVec2 grid_off, float rounding, int rounding_corners_flags)
{
    ...
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF)
    {
        ...
        for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++)
        {
            ...
            if (y2 <= y1)
                continue;
            for (float x = p_min.x + grid_off.x + (yi & 1) * grid_step; x < p_max.x; x += grid_step * 2.f)
            {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            float cos_hue_angle = ImCos(-H * 2.f * IM_PI);
            float sin_hue_angle = ImSin(-H * 2.f * IM_PI);
    ...
    ImU32 col32_no_alpha = ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 1.f));
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.f);
    }

    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = value_changed_sv ? 10.f : 6.f;
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max, IM_COL32(0,0,0,0), bar1_bb.GetWidth() / 2.f, ImVec2(0.f, 0.f));
        ...
        RenderArrowsForVerticalBar(draw_list, ImVec2(bar1_pos_x - 1, bar1_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.f);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    ImVec4 col_without_alpha(col.x, col.y, col.z, 1.f);
    float grid_step = ImMin(size.x, size.y) / 2.99f;
    float rounding = ImMin(g.Style.FrameRounding, grid_step * 0.5f);
    ImRect bb_inner = bb;
    float off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like a good middle ground to reduce those artifacts.
    bb_inner.Expand(off);
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.f)
    {
...
{
    ImGuiContext& g = *GImGui;
    return g.FontSize + (g.Style.FramePadding.x * 2.f);
}
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
    ...
    ImVec2 size;
    size.x = 0.f;
    size.y = GetTextLineHeightWithSpacing() * height_in_items_f + style.FramePadding.y * 2.f;
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        PushStyleVar(ImGuiStyleVar_ItemSpacing, style.ItemSpacing * 2.f);
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && g.OpenPopupStack.Size > g.BeginPopupStack.Size && g.OpenPopupStack[g.BeginPopupStack.Size].ParentWindow == window && !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            if (ImGuiWindow* next_window = g.OpenPopupStack[g.BeginPopupStack.Size].Window)
            {
                ...
                float extra = ImClamp(ImFabs(ta.x - tb.x) * 0.30f, 5.f, 30.f); // add a bit of extra slack.
...
ImGuiTabBar::ImGuiTabBar()
{
    ...
    ScrollingAnim = ScrollingTarget = 0.f;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    const float scrolling_speed = (tab_bar->PrevFrameVisible + 1 < g.FrameCount) ? FLT_MAX : (g.IO.DeltaTime * g.FontSize * 70.f);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    const ImVec2 arrow_button_size(g.FontSize - 2.f, g.FontSize + g.Style.FramePadding.y * 2.f);
    const float scrolling_buttons_width = arrow_button_size.x * 2.f;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (hovered && g.HoveredIdNotActiveTimer > 0.50f && bb.GetWidth() < tab->WidthContents)
    {
        ...
        bb.Max.x = bb.Min.x + (float)(int)ImLerp(bb.GetWidth(), tab->WidthContents, ImSaturate((g.HoveredIdNotActiveTimer - 0.40f) * 6.f));
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ImGuiContext& g = *GImGui;
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = ImVec2(label_size.x + g.Style.FramePadding.x, label_size.y + g.Style.FramePadding.y * 2.f);
    if (has_close_button)
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
    ...
    if (close_button_visible)
    {
        ...
        text_pixel_clip_bb.Max.x -= close_button_sz * 2.f;
    }
